// ================================================================
// Cyberland Ultra-Premium Bot + Dashboard (FULL FIXED)
// ================================================================
...
if (fs.existsSync(SETTINGS_PATH)) {
  try {
    settings = { ...settings, ...JSON.parse(fs.readFileSync(SETTINGS_PATH)) };
  } catch (e) {
...
app.post("/login", (req, res) => {
  ...
  if (username && password && ADMINS.includes(username.toLowerCase()) && password === ADMIN_PASSWORD) {
    req.session.user = username;
    return res.redirect("/");
  }
  res.send("❌ Invalid login. <a href='/login'>Try again</a>");
});
app.get("/", (req, res) => req.session.user ? res.sendFile(path.join(__dirname, "dashboard.html")) : res.redirect("/login"));
...
io.on("connection", (socket) => {
  ...
  socket.on("setChannel", (id) => { settings.channelId = id; saveSettings(); socket.emit("msg", "Channel set to: " + id); });
  socket.on("toggleAI", () => { settings.aiEnabled = !settings.aiEnabled; saveSettings(); socket.emit("msg", "AI is now: " + (settings.aiEnabled ? "✅ Enabled" : "❌ Disabled")); });
  ...
  socket.on("startUpdate", async () => {
    ...
    } catch (err) { socket.emit("msg", "❌ Error: " + err.message); }
  });
...
client.on("messageCreate", async (msg) => {
  ...
  if (!settings.aiEnabled || !settings.channelId || msg.channel.id !== settings.channelId) return;

  try {
    if (process.env.OPENAI_API_KEY) {
      const res = await axios.post(
        "https://api.openai.com/v1/chat/completions",
        { model: "gpt-3.5-turbo", messages: [{ role: "user", content: msg.content }], max_tokens: 300, temperature: 0.7 },
  ...
  } catch (err) { msg.reply("⚠️ AI error or unavailable"); }
});
...
const dashHTML = `
<!DOCTYPE html><html><head><title>Dashboard</title><script src="/socket.io/socket.io.js"></script>
<style>body{background:#0f172a;color:white;font-family:sans-serif;padding:20px;}button{margin:5px;padding:10px;border:none;border-radius:6px;background:#38bdf8;color:white;cursor:pointer;}#log{margin-top:20px;background:#1e293b;padding:10px;border-radius:6px;height:200px;overflow:auto;}</style></head>
<body>
...
client.login(process.env.DISCORD_TOKEN).then(()=>console.log("🤖 Discord bot logged in"));
